  ...
  CommandLineRunner seedRunner(UserRepo userRepo, UserService userService) {
    return args -> {
      ...
      userService.saveAuthUser(
          new AuthUser(
              null, "alicesmith", "321", new ArrayList<>(), new Student("Alice", "Smith"), null));
      userService.saveAuthUser(
          new AuthUser(
              null, "bobsmith", "123", new ArrayList<>(), new Student("Bob", "Smith"), null));
      userService.addRoleToUser("alicesmith", Role.ROLE_STUDENT);
      userService.addRoleToUser("bobsmith", Role.ROLE_STUDENT);

      userService.saveAuthUser(
          new AuthUser(
              null, "liamthomson", "321", new ArrayList<>(), null, new Tutor("Liam", "Thomson")));
      userService.saveAuthUser(
          new AuthUser(
              null, "olivathomson", "123", new ArrayList<>(), null, new Tutor("Oliva", "Thomson")));
